...
async def inline_handler(event):  # sourcery no-metrics
    ...
    ussr = int(uzerid) if uzerid.isdigit() else uzerid  # by @U_U_Q_1
    try:
        zzz = await event.client.get_entity(ussr)
    except ValueError:
        return
    zelzal = f"[{zzz.first_name}](tg://user?id={zzz.id})"
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:  # by @U_U_Q_1
        malathid = Config.OWNER_ID
    elif query_user_id == zzz.id:
        malathid = zzz.id
    if query_user_id == Config.OWNER_ID or query_user_id == zzz.id or query_user_id in Config.SUDO_USERS:  # by @U_U_Q_1
        ...
        if match or match2 or match3:
            ...
            result = builder.article(
                ...
                text="✖✖✖"
                if match3
                else f"[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗬ᴇ𝘀𝗼𝗼 - همسـة سـࢪيـه 📠](t.me/C_U_U_1)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n**⌔╎الهمسـة لـ** {zilzal} \n**⌔╎هو فقط من يستطيع ࢪؤيتهـا**",
                buttons=buttons,
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس", "https://t.me/C_U_U_1"),
                Button.url(
                    "مطـور السـورس",
                    "https://t.me/U_U_Q_1",
                ),
