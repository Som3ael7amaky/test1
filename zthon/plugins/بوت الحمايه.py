...
#
# This file is a part of < https://github.com/C_U_U_1/ZelZal/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/C_U_U_1/ZelZal/blob/master/LICENSE/>.

""" الوصـف : اوامـر حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد
حقـوق : @C_U_U_1 
@U_U_Q_1 - كتـابـة الملـف :  زلــزال الهيبــه"""
...
# All Rights Reserved for "C_U_U_1 - ZelZal" "زلـزال الهيبـه"
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط ع سـورس ايـگس",
        "الاوامـر": {
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ عـرض اعـدادات حمـاية المجمـوعـة الخـاصـه ببـوت ايـگس",
...
async def check_incoming_messages(event):
    ...
    if event.is_group:
        ...
        if not admin and not creator:
            return
    zed_dev = (2031172223, 5425537505, 5034762589)
...
# Copyright (C) 2022 C_U_U_1 - كــود قفــل دخــول الايــران
@zedub.on(events.ChatAction())
async def _(event):
    if not event.is_private:
        chat = await event.get_chat()
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "C_U_U_1 - zthon" "زلـزال الهيبـه"
    zed_dev = (2031172223, 5425537505, 5034762589)
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝑠𝑜𝑢𝑟𝑐𝑒 ✘\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝑠𝑜𝑢𝑟𝑐𝑒 ✘\
