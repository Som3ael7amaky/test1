    ...
    while AUTOBIOSTART:
        ...
        for normal in HM:
            if normal in normzltext:
              ...
              namefont = namerzfont[normzltext.index(normal)]
              HM = HM.replace(normal, namefont)
        DEFAULTUSERBIO = gvarstatus("DEFAULT_BIO") or "الحمد الله على كل شئ - @C_U_U_1"
...
async def _(event):
    ...
    await edit_delete(event, "**- تـم تفـعيل البروفايل الوقتي لحسابك .. بنجـاح 𓆰**")
...
async def _(event):
    ...
    await edit_delete(event, "**- تـم بـدء الاسـم الوقتـي لحسابك .. بنجـاح 𓆰**")
    await autoname_loop()
...
async def _(event):
    ...
    await edit_delete(event, "**- تـم تفـعيل الـنبذة الوقتيـه لحسابك .. بنجـاح 𓆰**")
    await autobio_loop()
