...
async def zedgban(event):  # sourcery no-metrics
    ...
    if user.id in zel_dev:
        return await edit_delete(zede, "**⪼ عـذراً ..لا استطيـع حظـࢪ مـطور السـورس عام ي بـآف 𓆰**")
    if user.id == 2031172223 or user.id == 5425537505 or user.id == 5034762589:
...
async def startgmute(event):
    if event.is_private:
        ...
        if user.id in zel_dev:
            return await edit_or_reply(event, "**- عــذࢪاً .. لايمكــنك كتــم مـطور السـورس عــام ؟!**")
        if user.id == 2031172223 or user.id == 5425537505 or user.id == 5034762589:
